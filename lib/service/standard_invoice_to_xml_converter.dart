...
class InvoiceToXmlConverter {
  static String generateXml(StandardInvoice invoice) {
    ...
    builder.element('Invoice', namespaces: {'cbc': 'urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2'}, nest: () {
      builder.element('cbc:InvoiceTypeCode', nest: invoice.invoiceType);
      ...
      builder.element('AccountingCustomerParty', nest: () {
        builder.element('Party', nest: () {
          builder.element('cbc:Name', nest: invoice.buyer.name);
          builder.element('cbc:VATNumber', nest: invoice.buyer.vatNumber);
          builder.element('cbc:Address', nest: invoice.buyer.address);
        });
      });
