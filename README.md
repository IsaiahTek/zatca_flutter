...
If you want to use the signed document for reporting (as required for Standard Invoice/Note reporting) then you might omit the `isForComplianceCheck` or explicitly indicate it's for reporting by passing in `isForComplianceCheck:false` to the `FatooraService.signInvoice()` method.
