...
class _SetupState extends State<Setup> {
  ...
  final TextEditingController _nameContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.name);
  final TextEditingController _addressContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.address);
  final TextEditingController _taxIdContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.taxId);
  final TextEditingController _buildingNumberContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.buildingNumber);
  final TextEditingController _citySubdivisionContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.citySubdivision);
  final TextEditingController _cityContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.city);
  final TextEditingController _postalZoneContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.postalZone);
  final TextEditingController _countryCodeContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.countryCode);
  final TextEditingController _schemeIDContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.schemeID.toString());
  final TextEditingController _companyIdContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.companyID);
  final TextEditingController _businessIdContr = TextEditingController(text: LocalStore.instance.myBusinessInfo?.businessID);
  ...
  TextEditingController ccsidBinarySecurityTokenController = TextEditingController(text: LocalStore.instance.ccsid?.token);
  TextEditingController ccsidSecretController = TextEditingController(text: LocalStore.instance.ccsid?.secret);
  TextEditingController ccsidRequestIdController = TextEditingController(text: LocalStore.instance.ccsid?.requestID.toString());

    TextEditingController pcsidBinarySecurityTokenController = TextEditingController(text: LocalStore.instance.pcsid?.token);
  TextEditingController pcsidSecretController = TextEditingController(text: LocalStore.instance.pcsid?.secret);
  TextEditingController pcsidRequestIdController = TextEditingController(text: LocalStore.instance.pcsid?.requestID.toString());
                ...
                Expanded(
                  child: TabBarView(
                    children: [
                        ...
                        child: Column(
                          ...
                          children: [
                                    ...
                                    onPressed: () async {
                                      ...
                                      debugPrint(
                                          ...
                                          FatooraServiceCsrResponse response = await controller.generateCsr(
                                              ...
                                              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("${response.response.status.name.toUpperCase()} for Generated files: (${response.csrOutputFileName.toString()} and ${response.keyOutputFileName})")));
                                      controller.csr =
                            ...
                            Row(
                              children: [
                                Tfield(
                                  label: 'Generated ES Secp256k1 Private Key (PEM)',
                                  width: 830,
                                  // readOnly: true,
                                  minline: 4,
                            ...
                            Row(
                              children: [
                                ElevatedButton(
                                    onPressed: () {
                                      debugPrint("NEW VALUE BEFORE CCSID REQ: ${_generatedCsrController.text}");
                                      controller.csr = _generatedCsrController.text;
                                      DefaultTabController.of(tabControllercontext).animateTo(2);
                                    },
                                    child: Text("Next")
                                ),
                              ],
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Tfield(width: 200, label: 'CCSID OTP', controller: otpController,),
                                SizedBox(
                                  width: 10,
                                ),
                                ElevatedButton(
                                    onPressed: () async {
                                      CCSIDRequestProp prop = CCSIDRequestProp(csr: controller.csr??"", otp: otpController.text);
                                      
                                      await controller.makeCCSIDRequest(prop);

                                      if(controller.ccsidData != null){
                                        ccsidSecretController.text = controller.ccsidData?.secret??"";
                                        ccsidBinarySecurityTokenController.text = controller.ccsidData?.binarySecurityToken??"";
                                        ccsidRequestIdController.text = controller.ccsidData?.requestID.toString()??"";
                                      }
                            ...
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.end,
                              children: [
                                Tfield(width: 300, label: 'Token Secret', readOnly: true, controller: ccsidSecretController,),
                                SizedBox(
                                  width: 10,
                                ),
                                Tfield(width: 300, label: 'Request ID', readOnly: true, controller: ccsidRequestIdController,),
                                SizedBox(
                                  width: 10,
                                ),
                                ElevatedButton(
                                    onPressed: () async {
                                      await controller.makePCSIDRequest(PCSIDRequestProp(binarySecurityToken: controller.ccsidData?.binarySecurityToken??"", requestId: controller.ccsidData?.requestID.toString()??"", secret: controller.ccsidData?.secret??""));
                                      pcsidBinarySecurityTokenController.text = controller.pcsidData?.binarySecurityToken??"";
                                      pcsidRequestIdController.text = controller.pcsidData?.requestID.toString()??"";
                                      pcsidSecretController.text = controller.pcsidData?.secret??"";
                            ...
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.end,
                              children: [
                                Tfield(width: 300, label: 'PCSID Secret', readOnly: true, controller: pcsidSecretController,),
                                SizedBox(
                                  width: 10,
                                ),
                                Tfield(width: 300, label: 'Request Date', readOnly: true, controller: pcsidRequestIdController,),
                              ],
                            ),
                            SizedBox(
                              height: 25,
                            ...
                            Row(
                              children: [
                                ElevatedButton(
                                    onPressed: () async {
                                      await controller.getCertAndSaveForSigning(certAndKey: CertAndKey(cert: ccsidBinarySecurityTokenController.text, key: _privateKeyController.text));
                                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Downloaded and saved! You can start signing invoices/debit notes/credit notes")));
                      ...
                      simplifiedInvoice.generateAndSaveXml('simple_invoice.xml');
                      ...
                      ComplianceInvoiceCheckResponse? res = await controller.request.requestComplianceCheck(prop: invoiceApi.invoiceRequest!);

                      debugPrint("RESULT FROM CHECKS ${res?.statusCode} ${res?.reportingStatus} ${res?.validationResults?.infoMessages?.map((d)=>d.toJson())}");
                                    },
                                    child: Text('Register & Download Certificate'))
                              ],
                            )
                          ],
                        ),
